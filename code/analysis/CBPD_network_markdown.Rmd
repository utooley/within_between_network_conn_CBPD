...
Interestingly, as a side note, the number of communities detected using modularity maximization on this data decreases slightly with age (range in k is `r range(main_unique$num_comms_modul)`), but that looks to just be that we don't have many samples above 8 years old.
...
for (net in nets){
  ...
  formula<-formula(paste0(net, '~age_scan+male+fd_mean+avgweight+pctSpikesFD+size_t+ses_composite+childaces_sum_ignorenan'))
  assign(name, lm(formula, data=main_unique))
  name2<-paste0("lm_paredu_",net)
  formula<-formula(paste0(net, '~age_scan+male+fd_mean+avgweight+pctSpikesFD+size_t+parent1_edu+childaces_sum_ignorenan'))
  assign(name2, lm(formula, data=main_unique))
  name3<-paste0("lm_income_",net)
  formula<-formula(paste0(net, '~age_scan+male+fd_mean+avgweight+pctSpikesFD+size_t+income_median+childaces_sum_ignorenan'))
...
Networks that showed marginal effects of the environment were MOT-FPN, *p_FDR*=`r networks_ses_pvals_fdr$networks_ses_pvals_fdr[networks_ses_pvals_fdr$names.main_unique...50.77.. =="sys2to6"]`, VAN-LIM *p_fdr*=`r networks_ses_pvals_fdr$networks_ses_pvals_fdr[networks_ses_pvals_fdr$names.main_unique...50.77.. =="sys4to5"]`, and LIM-LIM *p_fdr= `r networks_ses_pvals_fdr$networks_ses_pvals_fdr[networks_ses_pvals_fdr$names.main_unique...50.77.. =="sys5to5"]`.
