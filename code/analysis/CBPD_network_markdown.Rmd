...
hist((main_unique$motionDVCorrFinal), main=mean(main_unique$motionDVCorrFinal), xlab=pipeline, xlim = c(-0.1,0.7))
...
See below for histograms of the number of un-spike-regressed volumes we have in Run 1 (or the first run) only with different pipelines. *Need to pull these data from the cluster/work computer for run1 only*.
```{r censoring data loss,  echo=FALSE, include=TRUE,fig.show='hold', out.width='100%'}
#par(mfrow=c(2,2))
# pipelines=c('gsr_spkreg_fd0.5dvars1.75_drpvls', "gsr_censor_5contig_fd0.5dvars1.75_drpvls", "gsr_censor_5contig_fd1.25dvars2_drpvls", "nogsr_spkreg_fd1.25dvars2_drpvls")
# for (pipeline in pipelines){
# network_dir=paste0("~/Documents/projects/in_progress/within_between_network_conn_CBPD/data/imageData/",pipeline)
# load(paste0(network_dir,"/CBPD_n74_schaefer400_run1.Rdata"))
# main<- main %>% arrange(.,run) %>% group_by(ID) %>% filter(row_number() == 1)
# hist(main$size_t-main$nVolCensored, main=pipeline, xlab =paste0("# of subjects with > 120 volumes=", sum(main$size_t-main$nVolCensored>120), ", # of subjects pass QA criteria with > 120 volumes=",sum(main_unique$size_t-main_unique$nVolCensored>120)))
#}
```
...
# hist(main$totalSizet-main$totalnVolCensored, main=pipeline, xlab =paste0("# of subjects pass QA with > 120 volumes=", sum(main$totalSizet-main$totalnVolCensored>120)))
#}
```
...
Age x SES effects, not in the whole brain. *Sys1to3* (*p_fdr*=0.05), sys4to7 (ns), **sys3to7** (*p_fdr*=0.049).
...
Age x SES effects, not in the whole brain. **Sys1to3** (*p*=0.03), sys4to7 (ns), sys3to7 (ns).
...
Age x SES effects, not in the whole brain. **Sys1to3** (*p*=0.0004), sys4to7 (ns), *sys3to7* (*p*=0.10).
...
Age x SES effects, not in the whole brain. **Sys1to3** (*p*=0.003), sys4to7 (ns), sys3to7 (ns, *p*=0.20).
