...
#plot the difference
pvalues=c(0.001,0.0001,0.00001)
j <- hist(null_parcel_edges_only_0.001cosine)
l <- hist(sig_parcel_edges_only_0.001cosine)
...
parcelwise_age_sa_nonlin_pvals <- ifelse(parcelwise_age_sa_nonlin_pvals<0.05,parcelwise_age_sa_nonlin_pvals,NA)
...
similarity_metric<- cosine(edge_vector, SA_vector) #or correlation, cosine similarity, or mahalabonois distance? Other distance metrics?
...
hist(unlist(similarity_metric_perm), main = paste0("Cosine sim between FC edge age effects and SA pvalues, edge p < ",pvalue), 
     xlab = paste0("p=",p))
...
pvalue= 0.01#pvalue for edges to examine
indices <- which(edge_age_pvals_mat<pvalue, arr.ind = T) #indices of edges that have age effects  below a pval
l <- data.frame(table(indices)) #for each parcel, how many age-significant edges does it have at a given pval thresh
values <- vector(mode = "double", 400)
for (i in 1:400){
...
SA_vector=parcel_SA_pvals_fdr[-c(1,202),1] #parcel_SA_pvals uncorrected, or thresholded p-values
SA_vector=round(SA_vector,2) #need to round?

rotations<- rotate.parcellation(lh_centroids,rh_centroids,100)
perm.sphere.p(edge_vector,SA_vector, rotations, "pearson")
```
