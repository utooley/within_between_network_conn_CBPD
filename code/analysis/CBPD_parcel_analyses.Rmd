...
These values don't match those generated by mri_segstats, though, maybe because of the --accumulate flag I used that weights each parcel by the # of voxels? They are on different scales.
...
pvalue= 0.0001#pvalue for edges to examine
indices <- which(edge_age_pvals_mat<pvalue, arr.ind = T) #indices of edges that have age effects  below a pval
l <- data.frame(table(indices)) #for each parcel, how many age-significant edges does it have at a given pval thresh
values <- vector(mode = "double", 400)
for (i in 1:400){
...
#compare
edge_vector=values
SA_vector=parcel_SA_pvals_thresh[-c(1,202)] #take out the medial wall#parcel_SA_pvals, or thresh of either of these
SA_vector[is.na(SA_vector)] <- 0
dice_coef <- mclustcomp(edge_vector, SA_vector, type="sdc")
dice_coef_perm <- numeric(length = )
#create a null distribution
for (x in 1:1000){
  edge_vector_perm=sample(edge_vector, replace = F)
  SA_vector_perm=sample(SA_vector, replace = F)
  dice_coef_perm[x] <- mclustcomp(edge_vector_perm, SA_vector_perm, type="sdc")$scores
}
 hist(unlist(dice_coef_perm))
...
#make a big plot of them
par(mfrow=c(5,5))
lapply(which(edgewise_age_nonlin_pvals<0.00001)
```
