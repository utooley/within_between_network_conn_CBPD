...
col_fun = colorRamp2(breaks = c(-0.48,0,0.5),
...
#colormap
colFn_diverging = colorRampPalette(c("white","blue"));makecmap_options=list('colFn'=colFn_diverging)#, range= c(0,40))
...
num_of_sig_age_edges_lh=as.list(setNames(c(NA, parcelwise_age_sa_nonlin[2:201]), schaefer_atlas_region_names_lh))
num_of_sig_age_edges_rh=as.list(setNames(c(NA, parcelwise_age_sa_nonlin[203:402]), schaefer_atlas_region_names_rh))
#colormap
...
print(perm.sphere.p(edge_vector,parcel_SA_betas_pos, rotations, "pearson"))
}
...
## Average edge FC age effect within areas that show significant SA effects
```{r average beta for FC age effects within age-sig SA regions}
load("~/Documents/projects/in_progress/within_between_network_conn_CBPD/data/imageData/gsr_spkreg_fd0.5dvars1.75_drpvls/parcel_SA_age_n92.Rdata")
parcel_SA_pvals <- parcel_SA_pvals_fdr[-c(1,202),] 
parcel_SA_betas<- parcel_SA_betas[-c(1,202)] 
...
#indices <- which(parcel_SA_pvals[,1]<0.01, arr.ind = T) 
for (x in indices){
...
edge_effects_in_SA_areas <- c(colmax,rowmax)
colmax=vector();rowmax=vector()
#notindices <- sample(which(!parcel_SA_pvals[,1]<0.01, arr.ind = T), size=length(indices))
...
edge_effects_in_nonSA_areas <- c(colmax,rowmax)
print(t.test(edge_effects_in_SA_areas, edge_effects_in_nonSA_areas))
print(wilcox.test(edge_effects_in_SA_areas, edge_effects_in_nonSA_areas))

plt <- ggplot() + theme_classic()+
  geom_density(aes(x=edge_effects_in_SA_areas, ..count..),fill="purple", alpha=0.2) + 
  geom_density(aes(x=edge_effects_in_nonSA_areas,..count..),fill="red", alpha=0.2)+ ggtitle(paste0("Betas of age edge effects within areas showing sig \n pos age effects on SA at p <",pvalue)) +
