...
We controlled for sex, mean framewise displacement across both scans weighted by length of scan, percent of volumes censored for FD spikes in the data, the number of total volumes a participant had, and the average weight of the functional network across both scans (weighted by amount of good data per scan).
...
lm_sys1to3<- lm(sys1to3~age_scan+male+fd_mean_avg+avgweight+pctVolsCensored+totalSizet, data=main_unique)
lm_sys3to7<- lm(sys3to7~age_scan+male+fd_mean_avg+avgweight+pctVolsCensored+totalSizet, data=main_unique)
lm_sys4to7<- lm(sys4to7~age_scan+male+fd_mean_avg+avgweight+pctVolsCensored+totalSizet, data=main_unique)
