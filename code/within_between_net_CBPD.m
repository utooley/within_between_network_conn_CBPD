...
pipelines={'gsr_censor_5contig_fd1.25dvars2_drpvls','nogsr_spkreg_fd1.25dvars2_drpvls','gsr_censor_5contig_fd0.5dvars1.75_drpvls'} %add 'gsr_spkreg_fd0.5dvars1.75_drpvls'
...
header={'ID', 'run', 'avgweight', 'modul_avg', 'avgclustco_both','num_comms_modul_avg','part_coef_pos','part_coef_neg', 'system_segreg', 'mean_within_sys', 'mean_between_sys', 'sys1to1','sys1to2','sys1to3','sys1to4','sys1to5','sys1to6','sys1to7','sys2to1','sys2to2','sys2to3','sys2to4','sys2to5','sys2to6','sys2to7','sys3to1','sys3to2','sys3to3','sys3to4','sys3to5','sys3to6','sys3to7','sys4to1','sys4to2','sys4to3','sys4to4','sys4to5','sys4to6','sys4to7','sys5to1','sys5to2','sys5to3','sys5to4','sys5to5','sys5to6','sys5to7','sys6to1','sys6to2','sys6to3','sys6to4','sys6to5','sys6to6','sys6to7','sys7to1','sys7to2','sys7to3','sys7to4','sys7to5','sys7to6','sys7to7'}

outfile=table(char(subjlist.id0), char(subjlist.id1), avgweight, modul, avgclustco_both, num_comms_modul, part_coef_pos, part_coef_neg, system_segreg, mean_within_sys, mean_between_sys, system_conn)
outfile2=splitvars(outfile, 'system_conn')
outfile2.Properties.VariableNames=header

save(fullfile(outdir, strcat('n74_fixed_within_between_Yeo7_',parcellation,'_withmodulpartcoef.mat')), 'outfile2')
writetable(outfile2,fullfile(outdir,strcat('n74_fixed_within_between_Yeo7_',parcellation,'_withmodulpartcoef.csv')))
...
save(fullfile(outdir, strcat('n74_mean_system_conn_',parcellation,'.mat')), 'mean_system_conn_mat')

%save the nodal participation coefficient and clustering coefficient
outfile=dataset(char(subjlist.id0), char(subjlist.id1), part_coef_avg_all)
export(outfile,'File',strcat(outdir,'/n74_part_coef_avg_nodewise', parcellation,'.csv'),'Delimiter',',')

outfile=dataset(char(subjlist.id0), char(subjlist.id1), avgclustco_all)
export(outfile,'File',strcat(outdir,'/n74_clust_co_avg_nodewise', parcellation,'.csv'),'Delimiter',',')

    end
end

%% Make an average connectivity matrix for each pipeline and parcellation
% But only include each subject once (or I could average them together)
...
%export mean matrix
csvwrite(fullfile(outdir,strcat('average_FC_mat_n74_', parcellation,'.csv')), meanMatrix)
save(fullfile(outdir, strcat('average_FC_mat_n74_',parcellation,'.mat')), 'meanMatrix')
    end
end