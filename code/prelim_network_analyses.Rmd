...
main <- read.csv("~/Box/Mackey_Lab/CBPD (825656)/Data/Redcap Data/CBPD_data_190620_forJulia.csv") #find most recent CBPD data
...
```
## Data cleaning
Filter out runs from participants with < 50% of frames remaining after the 0.5mm and 1.75 DVARS thresholds, then melt it into wide format and average across participants with more than one run
```{r Data cleaning}

```

# Look at the distribution of motion in our sample


## Age and networks
Need to control for the amount of good data a participant has.
```{r Age and networks}
...
#Look at modularity and system segreg and participation coefficient
